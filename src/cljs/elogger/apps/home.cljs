(ns elogger.apps.home
  (:require
    [elogger.apps.auth.views :refer [login-form register-form]]
    [elogger.utils.components :refer [pretty-display form-group card]]
...
(defn jumbotron []
  [:div.page-header.header-filter
   {:style {"backgroundImage" "url('/img/profile_city.jpg')"}
    :data-parallax "true"}
   [:div.container
    [:div.row
     [:div.col-md-6
